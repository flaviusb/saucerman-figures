function [time, sol_y, sol_yp] = SaucermanIDA


%Initial Values and indication which vars have rates.
% Initial conditions and mass matrix
% b-AR/Gsa module
%   1       2       3       4       5       6       7           8       9
%   L       R       Gs      b1ARtot b1ARd   b1ARp   Gsagtptot   Gsagdp  Gsbg
y10=[0.988;     0.01079;3.829;  0.01205;0.0;    1.154e-3;0.02505;   6.446e-4;0.02569];
...
%End of Intial Values and indication which vars have rates.

neq = length(y0);

% Will probably need to set initial conditions on derivatives.  Hope note
% though.
...
sol_y(it,:) = y0';
sol_yp(it,:) = yp0';
[res, dummy1, status] = Saucerman_f(t0, y0, yp0);
yp0 = res; % Essentially, the above call would have calculated consistent initial values for the rates, if everything else was consistent.
%[yp0 id'] %Uncomment this to see the initial residuals vs the 1's and 0's indicating which are states and which are algebraics.
[res, dummy1, status] = Saucerman_f(t0, y0, yp0); % Call again, just to see if IC are consistent. Expect residuals to be 0.
%[yp0 id'] %Uncomment this to see the initial residuals vs the 1's and 0's indicating which are states and which are algebraics.
%Probably need to do even more work to get consistent initial conditions.
%Note that IDA's IC finding method is not available in the Matlab STB

% Note: does Malloc and sets initial conditions.
%IDAMalloc(@Saucerman_f,t0,y0,yp0,options);
IDAInit(@Saucerman_f,t0,y0,yp0,options);
...
function [res, flag, new_data] = Saucerman_f(t,y,yp)

%sprintf('inside compute rates')
t
% Parameters
...
% This line essentially converts from the mass matrix formulation to the
